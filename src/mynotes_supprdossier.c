	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
				...
				if(success==1)
					{
					sprintf(query,"select AES_DECRYPT(nomdossier,UNHEX(SHA2(\'%s\',512))) from dossier where compte=\'%s\' order by nomdossier asc;",mdpsave,nom);
					db_query(handler,query);
					}
				}
			}
...
if(success==1)
	...
	if(db_ntuples(result)>0)
		{
		printf("<form autocomplete=\"off\" action=\"/cgi-bin/mynotes_dosupprdossier.cgi\" method=\"post\" id=\"form-dosuppr\" data-transition=\"none\" data-rel=\"dialog\">\n\
		...
		for(i=0;i<db_ntuples(result);i++)
			...
			if(i==0) printf("<input type=\"radio\" name=\"radio-choice-r\" id=\"radio-choice-r%d\" value=\"%s\" data-theme=\"b\" checked=\"checked\">\n\
               <label for=\"radio-choice-r%d\">%s</label>\n",i,reponse,i,reponse);
			else printf("<input type=\"radio\" name=\"radio-choice-r\" id=\"radio-choice-r%d\" value=\"%s\" data-theme=\"b\">\n\
               <label for=\"radio-choice-r%d\">%s</label>\n",i,reponse,i,reponse);
			}
		printf("</fieldset>\n<input type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-theme=\"c\" data-icon=\"delete\" value=\"Supprimer ce dossier\">\n</form>\n");
		}
	db_clear_result(result);
	db_close(handler);
   printf("</div>\n\
