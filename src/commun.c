...
sprintf(buffer,"/tmp/mynotes_hash_in_%d",getpid());
fd=open(buffer,O_CREAT | O_TRUNC | O_WRONLY, 0644);
write(fd,entree,strlen(entree));
close(fd);
sprintf(buffer,"/usr/bin/openssl dgst -sha256 \'/tmp/mynotes_hash_in_%d\' > \'/tmp/mynotes_hash_out_%d\'",getpid(),getpid());
system(buffer);
sprintf(buffer,"/tmp/mynotes_hash_out_%d",getpid());
...
void conserve_retour_chariot(char *chaine)
{
int i,start;
char *res,buf[50];

start=i=0;
res=(char *)malloc(20000+strlen(chaine));
while(i<=strlen(chaine))
	{
...
conserve_retour_chariot(avatar);
url_decode(avatar);
prepare_contenu(avatar);
}
...
if(getenv("HTTP_COOKIE")==NULL || type==1) message_service("Oups.<br><br>MyNotes étant hautement sécurisé, cette page ne peut pas être rafraichie. <br><br>Désolé, il va falloir se reconnecter<br><a href=\"/mynotes/index.html\" class=\"ui-btn ui-btn-c ui-corner-all mc-top-margin-1-5\">Reconnexion</a>");
...
if(nb_cookies>0)
	...
	for(i=1;i<=nb_cookies;i++)
		...
		if(strcmp(cookie_name,"mynotesemail")==0) cookie_id=i;
		}
	if(cookie_id!=0)
		{
		...
		for(i=1;i<=nb_cookies;i++)
			{
			get_cookie_name(getenv("HTTP_COOKIE"),i,cookie_name);
			if(strcmp(cookie_name,"mynotesmdp")==0) cookie_id=i;
			}
		if(cookie_id!=0)
			{
			...
			for(i=1;i<=nb_cookies;i++)
				{
				get_cookie_name(getenv("HTTP_COOKIE"),i,cookie_name);
				if(strcmp(cookie_name,"mynotescode")==0) cookie_id=i;
				}
			if(cookie_id!=0)
				{
...
message_service("Oups.<br><br>MyNotes étant hautement sécurisé, cette page ne peut pas être rafraichie. <br><br>Désolé, il va falloir se reconnecter<br><a href=\"/mynotes/index.html\" class=\"ui-btn ui-btn-c ui-corner-all mc-top-margin-1-5\">Reconnexion</a>");
}