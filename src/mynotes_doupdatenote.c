...
char *nom,*mdp,*code,*mdpsav,*dossier,*dossierinitial,*titre,*titreinitial,*note,*horodatage,*jour,*mois,*annee,*heure,*minute,*seconde;
time_t tim;
struct tm *TM;
...
titreinitial=(char*)malloc(1000+query_size);
note=(char*)malloc(1000+query_size);
dossier=(char*)malloc(1000+query_size);
dossierinitial=(char*)malloc(1000+query_size);
...
tamb(code);
get_chaine(envoi,5,dossierinitial);
tamb(dossierinitial);
get_chaine(envoi,4,dossier);
tamb(dossier);
get_chaine(envoi,3,titreinitial);
tamb(titreinitial);
	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		{
         		sprintf(query,"delete from note where compte=\'%s\' and AES_DECRYPT(nomdossier,UNHEX(SHA2(\'%s\',512)))=\'%s\' and AES_DECRYPT(nomnote,UNHEX(SHA2(\'%s\',512)))=\'%s\';",nom,mdpsav,dossierinitial,mdpsav,titreinitial);
         		db_query(handler,query);
         		db_clear_result(result);
