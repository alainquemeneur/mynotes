	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
				...
				if(success==1)
					{
					sprintf(reponse,"openssl rand 10 | base32 > \'/tmp/mynotes_totp%s\'",nom);
					system(reponse);
					sprintf(reponse,"/tmp/mynotes_totp%s",nom);
					fd=open(reponse,O_RDONLY);
					nb=read(fd,reponse,100);
					close(fd);
					reponse[nb]=0;
					strcpy(lienretour,reponse);
					sprintf(reponse,"echo \'otpauth://totp/MyNotes (%s)?secret=%s\' | qrencode -t png -o \'/var/www/html/mynotes/qr/qr%s.png\'",nom,lienretour,nom);
					system(reponse);
...
<br><img src=\"/mynotes/qr/qr%s.png\"><br>Code secret : %s",nom,lienretour);
					}
				}
			}
		}
