...
char *nom,*mdp,*code,*mdpsav,*dossier,*titre;
	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		{
         		sprintf(reponse,"Confirmez-vous la suppression de la note \"%s\" ?\n\
         			<form action=\"/cgi-bin/mynotes_dosupprnote.cgi\" method=\"post\" id=\"form-main\" data-transition=\"none\" data-rel=\"dialog\">\n\
                  ...
                  <button type=\"submit\" data-mini=\"false\" data-inline=\"false\" data-theme=\"c\" data-icon=\"back\" class=\"ui-shadow\" style=\"max-width:100%%; border: 0; object-fit: contain;\">Confirmer</button>\n\
                  </form>\n",titre,nom,mdpsav,code,dossier,titre);
					}				
				db_close(handler);
				}
			}
