	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		{
         		sprintf(query,"delete from dossier where compte=\'%s\' and AES_DECRYPT(nomdossier,UNHEX(SHA2(\'%s\',512)))='%s';",nom,mdpsav,nomdossier);
					db_query(handler,query);
					db_clear_result(result);
					...
					if(db_ntuples(result)==0) 
						{
						strcpy(reponse,"Le dossier a bien été supprimé");
						db_clear_result(result);
						sprintf(query,"delete from note where compte=\'%s\' and AES_DECRYPT(nomdossier,UNHEX(SHA2(\'%s\',512)))='%s';",nom,mdpsav,nomdossier);
						db_query(handler,query);
						}
					else strcpy(reponse,"Erreur. Le dossier n'a pas été supprimé");
					db_clear_result(result);
					}				
				db_close(handler);
				}
