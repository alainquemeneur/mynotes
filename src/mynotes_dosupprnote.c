	...
	if(db_ntuples(result)==0)
		...
		if(db_ntuples(result)==0)
			...
			if(db_ntuples(result)==0)
         	...
         	if(success==1)
         		{
         		sprintf(query,"delete from note where compte=\'%s\' and AES_DECRYPT(nomdossier,UNHEX(SHA2(\'%s\',512)))=\'%s\' and AES_DECRYPT(nomnote,UNHEX(SHA2(\'%s\',512)))=\'%s\';",nom,mdpsav,dossier,mdpsav,titre);
         		db_query(handler,query);
         		db_clear_result(result);
         		strcpy(reponse,"La note a été supprimée");
					}				
				db_close(handler);
				}
			}
